...
def test_make_launcher_enables_91095_simt_for_sls_mixed_parallel_mode(
    ...
    assert src.count("triton_rt_launch_kernel(func, blockNum") == 2
...
def test_merged_code_preamble_shared_variables_present(
    ...
    for section_name, section in [("triton_launch_kernel", c_abi_launch), ("_launch", cpp_launch)]:
        ...
        assert "triton_rt_error_t ret = TRITON_RT_SUCCESS;" in section, f"{section_name}: missing ret"
...
def test_merged_code_taskqueue_mode_in_both_paths(
    ...
    assert c_abi_launch.count("std::function<triton_rt_error_t()> launch_call") == 1
    assert cpp_launch.count("std::function<triton_rt_error_t()> launch_call") == 1
